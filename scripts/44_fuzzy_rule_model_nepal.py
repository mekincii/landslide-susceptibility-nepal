"""
Advanced Baseline: Rule-Based Fuzzy Inference System (FIS)

GOAL: Implements a Mamdani-style Fuzzy Inference System to model landslide
susceptibility using geomorphological If-Then rules, providing a high-level
physical benchmark for machine learning performance.

RESPONSIBILITIES:
- Fuzzifies continuous environmental variables into 'Low', 'Medium', and
  'High' membership sets using adaptive quantile-based thresholds.
- Implements 10 core geomorphological rules (e.g., Slope-Precipitation
  interactions) using T-norm (min) conjunctions and S-norm (max) aggregations.
- Defuzzifies multi-class outputs into a single susceptibility score (0.0 - 1.0)
  using a discrete class-center approach.
- Benchmarks the qualitative 'Expert Logic' against the quantitative ML
  results within the standardized Spatial GroupKFold framework.
- Logs the baseline metrics to the master experiment tracker to facilitate
  final model selection and scientific justification.

INPUT:  data/processed/features_nepal_rxx_realxxx_model_hydro_river_precip_clean.parquet
OUTPUT: outputs/experiments/fuzzy_rule_spatial_cv.csv
"""
...
def build_fuzzy_rule_score(df: pd.DataFrame) -> np.ndarray:
...
def main():
    ...
    parser.add_argument("--out", default=r"outputs\experiments\fuzzy_rule_spatial_cv.csv")
    ...
    for fold, (_, te) in enumerate(gkf.split(df, y_true, groups), start=1):
        ...
        score = build_fuzzy_rule_score(df_te)
    ...
    print("\nFuzzy Rule Based Spatial CV Results")
    ...
    log_row = {
        ...
        "dataset": "terrain_hydro_river_precip_log_fuzzy_rule",
