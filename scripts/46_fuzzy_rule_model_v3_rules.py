"""
Comprehensive Expert Baseline: 22-Rule Fuzzy Inference System (V3)

GOAL: Completes the knowledge-driven modeling suite by implementing an
expanded, symmetrical rule base that explicitly defines Low, Medium, High,
and Very High susceptibility conditions.

RESPONSIBILITIES:
- Implements 22 geomorphological rules covering the full spectrum of risk,
  from extreme synergistic triggers (High Slope + High Rain) to singular
  stabilizing factors (Low Slope).
- Utilizes adaptive 3-quantile membership functions ($P_{20}, P_{50}, P_{80}$)
  to ensure features are normalized across the diverse Nepal landscape.
- Employs Mamdani 'Min-Max' inference to capture complex environmental
  interactions without the need for statistical training data.
- Provides the definitive 'Expert Baseline' against which all machine
  learning models (RF, Logistic Regression) are evaluated in the Spatial CV.
- Logs results to the master experiment tracker, enabling a 3-way
  comparison: Pure ML vs. Weighted Fuzzy vs. Rule-Based Fuzzy.

INPUT:  data/processed/features_nepal_rxx_realxxx_model_hydro_river_precip_clean.parquet
OUTPUT: outputs/experiments/fuzzy_rule_spatial_cv_v3_rules.csv
"""
...
def build_fuzzy_rule_score_v3(df: pd.DataFrame) -> np.ndarray:
    ...
    # High / very high risk rules
    # 1 slope high & precip high -> very high
    r1 = np.minimum(slope["high"], precip["high"])
    vhigh_out = np.maximum(vhigh_out, r1)

    # 2 slope high & spi high -> high
    r2 = np.minimum(slope["high"], spi["high"])
    high_out = np.maximum(high_out, r2)

    # 3 twi high & precip high -> high
    r3 = np.minimum(twi["high"], precip["high"])
    high_out = np.maximum(high_out, r3)

    # 4 elev high & precip high -> high
    r4 = np.minimum(elev["high"], precip["high"])
    high_out = np.maximum(high_out, r4)

    # 5 slope high & twi high -> high
    r5 = np.minimum(slope["high"], twi["high"])
    high_out = np.maximum(high_out, r5)

    # 6 precip high & spi high -> very high
    r6 = np.minimum(precip["high"], spi["high"])
    vhigh_out = np.maximum(vhigh_out, r6)

    # 7 slope med & precip high -> high
    r7 = np.minimum(slope["med"], precip["high"])
    high_out = np.maximum(high_out, r7)

    # 8 elev med & precip high -> high
    r8 = np.minimum(elev["med"], precip["high"])
    high_out = np.maximum(high_out, r8)

    # 9 slope high & precip med -> high
    r9 = np.minimum(slope["high"], precip["med"])
    high_out = np.maximum(high_out, r9)

    # 10 twi high & spi high -> high
    r10 = np.minimum(twi["high"], spi["high"])
    high_out = np.maximum(high_out, r10)

    # 11 elev high & slope high -> high
    r11 = np.minimum(elev["high"], slope["high"])
    high_out = np.maximum(high_out, r11)

    # 12 precip high & twi med -> high
    r12 = np.minimum(precip["high"], twi["med"])
    high_out = np.maximum(high_out, r12)

    # Medium risk rules
    # 13 slope med & precip med & twi med -> medium
    r13 = np.minimum(np.minimum(slope["med"], precip["med"]), twi["med"])
    med_out = np.maximum(med_out, r13)

    # 14 slope med & spi med -> medium
    r14 = np.minimum(slope["med"], spi["med"])
    med_out = np.maximum(med_out, r14)

    # 15 elev med & precip med -> medium
    r15 = np.minimum(elev["med"], precip["med"])
    med_out = np.maximum(med_out, r15)

    # 16 twi med & precip med -> medium
    r16 = np.minimum(twi["med"], precip["med"])
    med_out = np.maximum(med_out, r16)

    # Low risk rules
    # 17 slope low & precip low -> low
    r17 = np.minimum(slope["low"], precip["low"])
    low_out = np.maximum(low_out, r17)

    # 18 elev low & precip low -> low
    r18 = np.minimum(elev["low"], precip["low"])
    ...
    # 21 twi low & spi low -> low
    r21 = np.minimum(twi["low"], spi["low"])
    low_out = np.maximum(low_out, r21)

    # 22 slope low & twi low -> low
    r22 = np.minimum(slope["low"], twi["low"])
...
def main():
    ...
    parser.add_argument("--out", default=r"outputs\experiments\fuzzy_rule_spatial_cv_v3_rules.csv")
    ...
    for fold, (_, te) in enumerate(gkf.split(df, y_true, groups), start=1):
        ...
        score = build_fuzzy_rule_score_v3(df_te)
    ...
    print("\nFuzzy Rule Based V3 (Expanded Rules) Spatial CV Results")
    ...
    log_row = {
        ...
        "dataset": "terrain_hydro_river_precip_log_fuzzy_rule_v3_rules",
        ...
        "seed": "",
        "model": "Fuzzy_rule_based_v3_expanded_rules",
