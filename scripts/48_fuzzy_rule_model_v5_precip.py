"""
Precipitation-Dominant Expert Baseline: Fuzzy Inference System (V5)

GOAL: Evaluates the 'Monsoon Hypothesis' by implementing a fuzzy rule base
that prioritizes precipitation as the primary landslide trigger in Nepal.

RESPONSIBILITIES:
- Implements 20 refined geomorphological rules, with a primary focus on
  high-precipitation conjunctions and secondary terrain modulators.
- Introduces 'singular influence' rules (e.g., Rule 9) to model high-risk
  monsoon zones that may override local topographic stability.
- Utilizes the standardized Mamdani Min-Max inference framework and
  Center of Gravity defuzzification for 0.0-1.0 susceptibility scoring.
- Benchmarks the performance of 'Meteorological-First' logic against the
  general-purpose Random Forest model in a 20km Spatial GroupKFold CV.
- Finalizes the baseline suite, providing the most physically realistic
  expert-logic model for the Nepal study area.

INPUT:  data/processed/features_nepal_rxx_realxxx_model_hydro_river_precip_clean.parquet
OUTPUT: outputs/experiments/fuzzy_rule_spatial_cv_v5_precip.csv
"""
...
# Precipitation-dominant rule model
def build_fuzzy_rule_score_v5(df: pd.DataFrame) -> np.ndarray:
    ...
    # Core precipitation-dominant high-risk rules
    # 1 IF precip high AND slope high THEN very high
    r1 = np.minimum(precip["high"], slope["high"])
    vhigh_out = np.maximum(vhigh_out, r1)

    # 2 IF precip high AND spi high THEN very high
    r2 = np.minimum(precip["high"], spi["high"])
    vhigh_out = np.maximum(vhigh_out, r2)

    # 3 IF precip high AND twi high THEN high
    r3 = np.minimum(precip["high"], twi["high"])
    high_out = np.maximum(high_out, r3)

    # 4 IF precip high AND elev high THEN high
    r4 = np.minimum(precip["high"], elev["high"])
    high_out = np.maximum(high_out, r4)

    # 5 IF precip high AND slope med THEN high
    r5 = np.minimum(precip["high"], slope["med"])
    high_out = np.maximum(high_out, r5)

    # 6 IF precip med AND slope high THEN high
    r6 = np.minimum(precip["med"], slope["high"])
    high_out = np.maximum(high_out, r6)

    # 7 IF precip high AND twi med THEN high
    r7 = np.minimum(precip["high"], twi["med"])
    high_out = np.maximum(high_out, r7)

    # 8 IF precip high AND spi med THEN high
    r8 = np.minimum(precip["high"], spi["med"])
    high_out = np.maximum(high_out, r8)

    # 9 IF precip high THEN medium-high influence
    r9 = precip["high"]
    high_out = np.maximum(high_out, 0.8 * r9)

    # Supporting terrain/hydrology rules
    # 10 IF slope high AND spi high THEN high
    r10 = np.minimum(slope["high"], spi["high"])
    high_out = np.maximum(high_out, r10)

    # 11 IF slope high AND twi high THEN high
    r11 = np.minimum(slope["high"], twi["high"])
    high_out = np.maximum(high_out, r11)

    # 12 IF elev high AND slope high THEN high
    r12 = np.minimum(elev["high"], slope["high"])
    high_out = np.maximum(high_out, r12)

    # 13 IF slope med AND precip med AND twi med THEN medium
    r13 = np.minimum(np.minimum(slope["med"], precip["med"]), twi["med"])
    med_out = np.maximum(med_out, r13)

    # 14 IF elev med AND precip med THEN medium
    r14 = np.minimum(elev["med"], precip["med"])
    med_out = np.maximum(med_out, r14)

    # 15 IF spi med AND twi med THEN medium
    r15 = np.minimum(spi["med"], twi["med"])
    med_out = np.maximum(med_out, r15)

    # Low-risk rules
    # 16 IF precip low AND slope low THEN low
    r16 = np.minimum(precip["low"], slope["low"])
    low_out = np.maximum(low_out, r16)

    # 17 IF precip low THEN low
    r17 = precip["low"]
    low_out = np.maximum(low_out, r17)

    # 18 IF slope low THEN low
    r18 = slope["low"]
    low_out = np.maximum(low_out, r18)

    # 19 IF twi low AND spi low THEN low
    r19 = np.minimum(twi["low"], spi["low"])
    low_out = np.maximum(low_out, r19)

    # 20 IF elev low AND precip low THEN low
    r20 = np.minimum(elev["low"], precip["low"])
...
def main():
    ...
    parser.add_argument("--out", default=r"outputs\experiments\fuzzy_rule_spatial_cv_v5_precip.csv")
    ...
    for fold, (_, te) in enumerate(gkf.split(df, y_true, groups), start=1):
        ...
        score = build_fuzzy_rule_score_v5(df_te)
    ...
    print("\nFuzzy Rule Based V5 (Precip Dominant) Spatial CV Results")
    ...
    log_row = {
        ...
        "dataset": "terrain_hydro_river_precip_log_fuzzy_rule_v5_precip",
        ...
        "seed": "",
        "model": "Fuzzy_rule_based_v5_precip_dominant",
