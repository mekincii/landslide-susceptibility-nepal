"""
Fuzzy Baseline Tuning: Defuzzification Centroid Optimization

GOAL: Refines the knowledge-driven baseline by optimizing the output class
centers used in the defuzzification process to improve risk separation.

RESPONSIBILITIES:
- Implements a 10-rule geomorphological inference system using triangular
  and trapezoidal membership functions.
- Tunes the 'Defuzzification Centroids' ($c_{low}=0.10, c_{vhigh}=0.95$)
  to enhance the contrast between high-hazard and low-hazard zones.
- Utilizes the Center of Gravity (CoG) approach to aggregate linguistic
  activations into a continuous 0.0-1.0 susceptibility score.
- Benchmarks the tuned physical baseline against machine learning
  alternatives using the 20km Spatial GroupKFold framework.
- Persists performance metrics to the global experiment log to determine
  if better calibration can close the gap with the Random Forest.

INPUT:  data/processed/features_nepal_rxx_realxxx_model_hydro_river_precip_clean.parquet
OUTPUT: outputs/experiments/fuzzy_rule_spatial_cv_v4_defuzz.csv
"""
...
def main():
    ...
    parser.add_argument("--out", default=r"outputs\experiments\fuzzy_rule_spatial_cv_v4_defuzz.csv")
    ...
    log_row = {
        ...
        "seed": "",
        "model": "Fuzzy_rule_based_v4_defuzz",
